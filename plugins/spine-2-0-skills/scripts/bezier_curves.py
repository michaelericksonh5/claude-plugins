    ...
    for anim_name, anim in skel.get("animations", {}).items():
        ...
        for slot_name, tracks in anim.get("slots", {}).items():
            for timeline_name, keys in tracks.items():
                ...
                errors.extend(_validate_keys(path, keys, channel_count, export_profile))
        for skin_name, skin_tracks in anim.get("attachments", {}).items():
            for slot_name, slot_tracks in skin_tracks.items():
                for attachment_name, attachment_tracks in slot_tracks.items():
                    for timeline_name, keys in attachment_tracks.items():
                        path = (
                            f"animation {anim_name!r} attachment {skin_name} "
                            f"{slot_name} {attachment_name} {timeline_name}"
                        )
                        if timeline_name in {"sequence", "deform"}:
                            errors.extend(_validate_no_curve_fields(path, keys))
