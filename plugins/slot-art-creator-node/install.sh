...
if ! command -v node &>/dev/null; then
    ...
    echo ""
    echo " Install Node.js 20 or later:"
...
if [ "$NODE_MAJOR" -lt 20 ]; then
    echo " ERROR: Node.js 20+ required. You have $NODE_VER."
...
prune_staged_plugin() {
    ...
    for pattern in "${EXCLUDED_FILES[@]}"; do
        find "$root" -type f -name "$pattern" ! -name ".env.example" -delete
    done
}
...
package_cowork_zip() {
    ...
    echo " Legacy/admin Cowork upload ZIP created:"
    echo "   $output"
    echo " Use only if GitHub-synced marketplaces are blocked by org policy."
}

case "$install_target" in
    1) do_install_code ;;
    2) do_install_code; package_cowork_zip ;;
    3) package_cowork_zip ;;
    *) echo " Skipped. For Code and Cowork, use the plugin marketplace flow." ;;
esac
