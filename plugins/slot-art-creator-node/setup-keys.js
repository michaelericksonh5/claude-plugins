 ...
 *   smart_resize               -> Gemini (NB2 + local pngjs crop)
 *
 ...
 *
 * Keys are written to the plugin's canonical .env:
 *
 *   ~/.h5g-slot-art-creator/.env          <- Claude Desktop / Claude Code.
 *   %USERPROFILE%\.h5g-slot-art-creator\.env  (Windows)
 *
 * This survives plugin reinstalls and updates; the MCP server reads .env,
 * plugin env, and process env at runtime. To also mirror keys into Claude
 * Code's global settings env, pass --mirror-settings explicitly.
 *
 * Usage:
 ...
 *   node setup-keys.js --mirror-settings --gemini  # also write ~/.claude/settings.json
...
function mirrorIntoClaudeSettings(envVars) {
  if (!MIRROR_SETTINGS) return null;
  return mergeIntoClaudeSettings(envVars);
}
...
async function hiddenInput(question) {
  return new Promise((resolve) => {
    ...
    const onSigint = () => {
      if (cleanup()) process.stdout.write("\n");
      process.exit(130);
    };
...
async function cmdSetFal() {
  ...
  values["FAL_KEY"] = key;
  writeEnv(values);
  const claudePath = mirrorIntoClaudeSettings({ FAL_KEY: key });

  console.log(`\nWrote ${ENV_PATH}`);
  if (claudePath) console.log(`Wrote ${claudePath} (shared across all marketplace plugins)`);
  console.log("fal.ai will handle all 4 tools when it is the only NB2 key.");
  console.log("If you also set GEMINI_API_KEY, all four NB2 tools default to Gemini.");
  return 0;
}
...
async function cmdSetGemini() {
  ...
  const claudePath = mirrorIntoClaudeSettings({ GEMINI_API_KEY: key, GOOGLE_API_KEY: key });
  ...
  console.log("Gemini remains the default for smart_resize; unset GEMINI_API_KEY to force fal.ai.");
  return 0;
}
...
async function cmdSetOpenAI() {
  ...
  writeEnv(values);
  const claudePath = mirrorIntoClaudeSettings({ OPENAI_API_KEY: key });
...
async function cmdCheck() {
  ...
  // Check all supported locations; settings.json is a global convenience,
  // while .env is the plugin-specific durable store.
  const resolve = (k) =>
  ...
  if (!falKey && !geminiKey) {
    ...
    console.log("  smart_resize               -> Gemini (NB2 + local pngjs crop)");
  } else if (geminiKey) {
...
async function cmdClearFal() {
  ...
  values["FAL_KEY"] = "";
  writeEnv(values);
  const claudePath = mirrorIntoClaudeSettings({ FAL_KEY: "" });
  console.log(`Cleared FAL_KEY in ${ENV_PATH}${claudePath ? " and ~/.claude/settings.json" : ""}`);
  return 0;
}
...
async function cmdClearGemini() {
  ...
  writeEnv(values);
  const claudePath = mirrorIntoClaudeSettings({ GEMINI_API_KEY: "", GOOGLE_API_KEY: "" });
  console.log(`Cleared GEMINI_API_KEY in ${ENV_PATH}${claudePath ? " and ~/.claude/settings.json" : ""}`);
  return 0;
}
...
async function cmdClearOpenAI() {
  ...
  writeEnv(values);
  const claudePath = mirrorIntoClaudeSettings({ OPENAI_API_KEY: "" });
  console.log(`Cleared OPENAI_API_KEY in ${ENV_PATH}${claudePath ? " and ~/.claude/settings.json" : ""}`);
  return 0;
}
...
async function cmdInteractive() {
  ...
  console.log("======================================");
  console.log("Keys are written to ~/.h5g-slot-art-creator/.env (canonical for");
  console.log("this plugin). Use --mirror-settings only if you also want to");
  console.log("write Claude Code's global ~/.claude/settings.json env.");
  console.log("");
...
const MIRROR_SETTINGS = args.includes("--mirror-settings");
