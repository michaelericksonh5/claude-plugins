...
|---|---|---|---|
| `GEMINI_API_KEY` / `GOOGLE_API_KEY` | Gemini | **gemini-3.1-flash-image-preview** (NB2) | Default when Gemini is available; one Gemini call per target + pngjs center-crop locally. |
| Only `FAL_KEY` | fal.ai | **nano-banana-pro** | Single API call, purpose-built endpoint. Also available by unsetting Gemini for that process. |
...
Gemini version does it client-side here in Node (using pngjs for the crop,
no native deps), while the fal.ai version does it as one server-side call.
...
- **Either backend works.** Gemini is the default when its key is set —
  one Gemini call per target plus a pngjs center-crop. fal.ai is the
  fallback/force path when Gemini is unset and uses a single purpose-built
  endpoint call. Both produce pixel-perfect output.
