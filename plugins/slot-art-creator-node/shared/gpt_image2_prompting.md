...
| **Multi-aspect resize / smart-resize** | `nb2_smart_resize` (Gemini NB2 by default; fal.ai NB Pro when only `FAL_KEY` is set) | We prototyped a gpt-image-2-based smart-resize tool and didn't ship it because output quality wasn't verified against the NB2/fal paths. For multi-aspect resize, use nb2_smart_resize. |
...
| `/slot-step-10` (multi-aspect) | **NB2 always.** Use `nb2_smart_resize` (Gemini NB2 by default; fal.ai NB Pro when only `FAL_KEY` is set). A gpt-image-2-based equivalent was prototyped but not shipped — we couldn't verify the output quality against the well-tested NB2/fal paths. |
