...
**Skip for:** `/slot-step-00`, `/slot-step-01`, `/slot-setup`, `/slot-help`, `/slot-compare`.
...
5. **Don't lecture.** Briefly explain *why* the prerequisite is needed
   ...
   ask. Don't dump the full plugin tour unless the user explicitly asks
   for one.
...
and `/slot-compare` do not need an active project — they either create one
or handle the missing-project route themselves.
