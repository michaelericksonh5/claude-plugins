...
async function runFfmpeg(ffmpegBin, args) {
    await new Promise((resolve, reject) => {
        child_process.execFile(ffmpegBin, args, { timeout: 300_000 }, (err) => (err ? reject(new Error(`ffmpeg: ${err.message}`)) : resolve()));
    });
}
...
// ─── Register ────────────────────────────────────────────────────────────────
export function registerFfmpegTools(server) {
    // ── Convert Video ──────────────────────────────────────────────────────────
    server.registerTool("veo_convert_video", {
...
Returns: Path to the converted file and its size.`,
        ...
        try {
            ...
            if (!ffmpegBin) {
                return { content: [{ type: "text", text: "Error: ffmpeg not available. Generate a video first to trigger auto-install." }] };
            }
    ...
    // ── Resize Video ───────────────────────────────────────────────────────────
    server.registerTool("veo_resize_video", {
...
//# sourceMappingURL=ffmpegTools.js.map