...
let _ffmpegBin = null;
export async function ensureFfmpegStatic() {
    if (_ffmpegChecked)
        return _ffmpegBin;
    _ffmpegChecked = true;
...
function buildFilename(opts, result) {
    ...
    if (assetPart && typePart)
        return `${assetPart}_${typePart}`;
    if (assetPart)
        return assetPart;
...
// ─── Public API ───────────────────────────────────────────────────────────────
export async function formatResult(result, opts) {
    let movPath = null;
    let movNote = "";
    try {
...
//# sourceMappingURL=movConverter.js.map