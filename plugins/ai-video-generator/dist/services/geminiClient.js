 ...
 */
import { GEMINI_API_BASE, GEMINI_MODEL, GEMINI_POLL_INTERVAL_MS, GEMINI_MAX_POLL_ATTEMPTS, } from "../constants.js";
async function geminiPost(path, body, apiKey) {
...
async function geminiGet(path, apiKey) {
    const url = `${GEMINI_API_BASE}/${path}`;
    const res = await fetch(url, {
...
async function pollOperation(operationName, apiKey) {
    for (let attempt = 0; attempt < GEMINI_MAX_POLL_ATTEMPTS; attempt++) {
        ...
        const status = (await geminiGet(operationName, apiKey));
...
function extractGeminiVideo(op, model) {
...
export async function geminiTextToVideo(apiKey, params) {
    const videoConfig = {};
    if (params.aspect_ratio)
        videoConfig["aspectRatio"] = params.aspect_ratio;
    if (params.resolution)
        videoConfig["resolution"] = params.resolution;
    if (params.duration_seconds)
        videoConfig["durationSeconds"] = params.duration_seconds;
    if (params.negative_prompt)
        videoConfig["negativePrompt"] = params.negative_prompt;
    if (params.seed !== undefined)
        videoConfig["seed"] = params.seed;
    const body = {
        model: GEMINI_MODEL,
        prompt: params.prompt,
    };
    if (Object.keys(videoConfig).length > 0)
        body["config"] = videoConfig;
    const op = (await geminiPost(`models/${GEMINI_MODEL}:predictLongRunning`, body, apiKey));
    const completed = await pollOperation(op.name, apiKey);
    return extractGeminiVideo(completed, GEMINI_MODEL);
}
// ─── Gemini Image-to-Video ──────────────────────────────────────────────────
export async function geminiImageToVideo(apiKey, params) {
    const videoConfig = {};
    if (params.aspect_ratio)
        videoConfig["aspectRatio"] = params.aspect_ratio;
    if (params.resolution)
...
export async function geminiFirstLastFrameToVideo(apiKey, params) {
    ...
    if (params.first_frame_url)
        firstImg = await urlToBase64(params.first_frame_url);
    else if (params.first_frame_base64)
        firstImg = { data: params.first_frame_base64, mime: params.image_mime_type ?? "image/jpeg" };
    else
        throw new Error("Provide first_frame_url or first_frame_base64.");
    let lastImg;
    if (params.last_frame_url)
        lastImg = await urlToBase64(params.last_frame_url);
    else if (params.last_frame_base64)
        lastImg = { data: params.last_frame_base64, mime: params.image_mime_type ?? "image/jpeg" };
    const body = {
        model: GEMINI_MODEL,
        prompt: params.prompt,
...
export async function geminiReferenceToVideo(apiKey, params) {
    // Fetch and inline all reference images
    const referenceImages = await Promise.all(params.image_urls.map(async (url) => {
        const res = await fetch(url);
        const buf = await res.arrayBuffer();
...
export async function geminiExtendVideo(_apiKey, _params) {
    throw new Error("Video extension is not available via the Gemini REST API with an API key.\n\n" +
        "Reasons:\n" +
